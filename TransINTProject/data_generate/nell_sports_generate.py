...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct  6 15:10:51 2019

@author: TiffMin
"""

##
r_id2_vrr = {0: (0,1), 1:(0,1), 2: (0, 1), 3:(0,1)}
pickle.dump(r_id2_vrr, open('datasets/NELL_datasets/sports/r_id2_vrr.p','wb'))

tripleDict = {pair:1 for pair in test_triples_list + train_triples_list}
pickle.dump(tripleDict, open('datasets/NELL_datasets/sports/tripleDict.p','wb'))
...
pickle.dump(avg_tail_per_head, open('datasets/NELL_datasets/sports/tail_per_head.p', 'wb'))
pickle.dump(avg_head_per_tail, open('datasets/NELL_datasets/sports/head_per_tail.p', 'wb'))
...
pickle.dump(tripledict_given_head_rel, open('datasets/NELL_datasets/sports/tripledict_given_head_rel','wb'))
pickle.dump(tripledict_given_tail_rel, open('datasets/NELL_datasets/sports/tripledict_given_tail_rel','wb'))

#######
#Make Possible Triple Dicts
#First make equiv_child_dict
equiv_child_dict = {0:[(1,1), (2,1), (3,1)], 1:[(2,1)]}
...
pickle.dump(possible_imp_given_head_rel, open('datasets/NELL_datasets/sports/possible_imp_given_head_rel','wb'))
pickle.dump(possible_imp_given_tail_rel, open('datasets/NELL_datasets/sports/possible_imp_given_tail_rel','wb'))
...
def NULLassign_rank_to_tree():
    ...
    for r in [0,1,2,3]:
        if r ==0:
            rank_start[r] = 0
            rank_end[r] = 2
        #elif r in [1,3]: #the more general concept #How to deal?
        elif r in [1]:
            #Make two versions: 3 has rank 1, 3 has rank 2
            rank_start[r] = 0
            rank_end[r] = 1
        else:
            rank_start[r] = 0
...
def NULLassign_random_mats(vvr_bundle_heads, rank_end):
    ...
    #for l in [1,3]: #less general ones
    for l in [1]:
        dict_of_linenar_comb[l] = nn.Parameter(torch.randn(2,3), requires_grad = True)
    #for l in [2]: #or three?
    for l in [2,3]:
        dict_of_linenar_comb[l] = nn.Parameter(torch.randn(1,3), requires_grad = True)
...
pickle.dump(hr_gap1_dict, open('/data/scratch-oc40/symin95/thesis_kg_project/transint/knowledge_representation_pytorch-master/datasets/NELL_datasets/sports/three_2_NULL_dim_50_hr_gap1_dict.p', 'wb'))
