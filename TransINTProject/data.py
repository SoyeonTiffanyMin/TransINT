...
from utils import Triple, implication_correct
...
# If it is, regenerate.
def corrupt_head_filter(triple, entityTotal, possible_imp_given_tail_rel, possible_imp_given_head_rel, tripleDict, impfilter=False):
    newTriple = deepcopy(triple)
    while True:
        newHead = random.randrange(entityTotal)
        if impfilter == True:
            if not(implication_correct(newHead, newTriple.t, newTriple.r, possible_imp_given_tail_rel, possible_imp_given_head_rel)):
                break
        else:
...
# with checking whether it is a false negative sample.
# If it is, regenerate.
def corrupt_tail_filter(triple, entityTotal, possible_imp_given_tail_rel, possible_imp_given_head_rel, tripleDict, impfilter=False):
    newTriple = deepcopy(triple)
    while True:
        newTail = random.randrange(entityTotal)
        if impfilter == True:
            if not(implication_correct(newTriple.h, newTail, newTriple.r, possible_imp_given_tail_rel, possible_imp_given_head_rel)):
                break
        else:
...
# Corrupt the head or tail according to Bernoulli Distribution,
# with checking whether it is a false negative sample.
def corrupt_filter_two_v2(triple, entityTotal, tail_per_head, head_per_tail,possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict, impfilter):
    rel = triple.r
    split = tail_per_head[rel] / (tail_per_head[rel] + head_per_tail[rel])
    random_number = random.random()
    if random_number < split:
        newTriple = corrupt_head_filter(triple, entityTotal, possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict, impfilter)
    else:
        newTriple = corrupt_tail_filter(triple, entityTotal,possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict, impfilter)
    return newTriple
...
def getBatch_filter_random(tripleList, batchSize, entityTotal, possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict,impfilter):
    oldTripleList = random.sample(tripleList, batchSize)
    newTripleList = [corrupt_head_filter(triple, entityTotal,possible_imp_given_tail_rel, possible_imp_given_head_rel, impfilter) if random.random() < 0.5 
...
# and generate negative samples by corrupting head or tail with equal probabilities,
# with checking whether false negative samples exist.
def getBatch_filter_all(tripleList, entityTotal,possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict,impfilter):
    newTripleList = [corrupt_head_filter(triple, entityTotal, possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict, impfilter) if random.random() < 0.5 
        else corrupt_tail_filter(triple, entityTotal, possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict, impfilter) for triple in tripleList]
    ph, pt ,pr = getThreeElements(tripleList)
    nh, nt, nr = getThreeElements(newTripleList)
    return ph, pt, pr, nh, nt, nr
...
# and generate negative samples by corrupting head or tail according to Bernoulli Distribution,
# with checking whether false negative samples exist.
def getBatch_filter_random_v2(tripleList, batchSize, entityTotal, tail_per_head, head_per_tail,possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict,  impfilter):
    oldTripleList = random.sample(tripleList, batchSize)
    newTripleList = [corrupt_filter_two_v2(triple, entityTotal, tail_per_head, head_per_tail, possible_imp_given_tail_rel, possible_imp_given_head_rel, tripleDict,impfilter) 
        for triple in oldTripleList]
    ph, pt ,pr = getThreeElements(oldTripleList)
    nh, nt, nr = getThreeElements(newTripleList)
    return ph, pt, pr, nh, nt, nr
...
# and generate negative samples by corrupting head or tail according to Bernoulli Distribution,
# with checking whether false negative samples exist.
def getBatch_filter_all_v2(tripleList, entityTotal,tail_per_head, head_per_tail,possible_imp_given_tail_rel, possible_imp_given_head_rel, tripleDict, impfilter):
    newTripleList = [corrupt_filter_two_v2(triple, entityTotal, tail_per_head, head_per_tail, possible_imp_given_tail_rel, possible_imp_given_head_rel,tripleDict, impfilter) 
        for triple in tripleList]
    ph, pt ,pr = getThreeElements(tripleList)
    nh, nt, nr = getThreeElements(newTripleList)
